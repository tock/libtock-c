...
{
  syscall_return_t res = command(DRIVER_NUM_PROCLOADER_KERNEL, COMMAND_PROC_SET_DYNAMIC_START_ADDRESS, 0, 0);
  return tock_command_return_novalue_to_returncode(res);
}
...
// Commnad to get FLASH end address
int ota_get_end_address_of_writable_flash(uint32_t *endaddress)
{
  ...
  return tock_command_return_u32_to_returncode(res, (uint32_t *)endaddress);
}
...
// Commnad to get the maximum number of process that kernel can handle
int ota_get_process_maxnum(uint32_t *processes_maxnum)
{
  syscall_return_t res = command(DRIVER_NUM_PROCLOADER_KERNEL, COMMAND_PROC_GET_PROCESS_MAX, 0, 0);
  return tock_command_return_u32_to_returncode(res, (uint32_t *)processes_maxnum);
}
...
// Commnad to let the kernel use the end address of sram that OTA_app uses as the start address fo unused sram
