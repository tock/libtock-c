...
#define TOCK_SHA_VERIFY_CB 1

#define TOCK_SHA_DATA_BUF     0
#define TOCK_SHA_COMPARE_BUF  1
...
returncode_t libtock_sha_set_verify_upcall(subscribe_upcall callback, void* opaque) {
  subscribe_return_t sval = subscribe(DRIVER_NUM_SHA, TOCK_SHA_VERIFY_CB, callback, opaque);
  return tock_subscribe_return_to_returncode(sval);
}
...
returncode_t libtock_sha_set_readonly_allow_compare_buffer(uint8_t* buffer, uint32_t len) {
  allow_ro_return_t aval = allow_readonly(DRIVER_NUM_SHA, TOCK_SHA_COMPARE_BUF, (void*) buffer, len);
  return tock_allow_ro_return_to_returncode(aval);
}
...
returncode_t libtock_sha_command_verify(void) {
  syscall_return_t cval = command(DRIVER_NUM_SHA, TOCK_SHA_VERIFY, 0, 0);
  return tock_command_return_novalue_to_returncode(cval);
}
