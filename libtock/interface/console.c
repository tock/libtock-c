...
static void read_done_upcall(int   status,
                             int   length,
                             int   _z __attribute__ ((unused)),
                             void* ud) {
  libtock_console_callback_read cb = (libtock_console_callback_read)ud;
  cb(tock_status_to_returncode(status), length);
}

static void write_done_upcall(int   length,
...
returncode_t libtock_console_write(const uint8_t* buffer, uint32_t len, libtock_console_callback_write cb) {
  ...
  err = libtock_console_write_done_set_upcall(write_done_upcall, (void*)cb);
...
returncode_t libtock_console_read(uint8_t* buffer, uint32_t len, libtock_console_callback_read cb) {
  ...
  err = libtock_console_read_done_set_upcall(read_done_upcall, (void*)cb);
  if (err != RETURNCODE_SUCCESS) return err;
