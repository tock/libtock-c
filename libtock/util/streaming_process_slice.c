...
returncode_t streaming_process_slice_init(
  ...
  state->current_app_buffer = false; // false -> buffer_a
...
returncode_t streaming_process_slice_get_and_swap(
  ...
  if (allow_res.success) {
    ...
    state->current_app_buffer = !state->current_app_buffer;
...
returncode_t streaming_process_slice_deinit(
  ...
  uint8_t**                        buffer_b,
  size_t*                          size_b) {
  uint8_t* ret_buffer_a, * ret_buffer_b;
  size_t ret_size_a, ret_size_b;
  ...
  if (unallow_res.success) {
    ...
    // kernel halves:
    if (!state->current_app_buffer) {
      // `current_app_buffer` == false -> app_buffer_ptr == buffer_a
      ret_buffer_a = state->app_buffer_ptr;
      ret_size_a   = state->app_buffer_size;
      ret_buffer_b = unallow_res.ptr;
      ret_size_b   = unallow_res.size;
    } else {
      // `current_app_buffer` == true -> app_buffer_ptr == buffer_b
