...
/******************************************************************************************************
* Function to uninstall an application binary
*
* Takes the app_handle for the binary and the callback function as arguments
******************************************************************************************************/

returncode_t libtock_app_loader_uninstall_with_app_handle(uint32_t app_handle, subscribe_upcall cb) {
  // set up the unload done callback
  int err = libtock_app_loader_set_uninstall_with_app_handle_upcall(cb, NULL);
  if (err != 0) {
    printf("[Error] Failed to set uninstall done callback: %d\n", err);
    return err;
  }

  return libtock_app_loader_command_uninstall_with_app_handle(app_handle);
}
