...
returncode_t libtocksync_sha_hash_verification(libtock_sha_algorithm_t hash_type,
  ...
  ret = libtock_sha_set_readonly_allow_compare_buffer(compare_buffer, hash_length);
  if (ret != RETURNCODE_SUCCESS) return ret;
  defer { libtock_sha_set_readonly_allow_compare_buffer(NULL, 0);
  }

  ret = libtock_sha_command_verify();
  if (ret != RETURNCODE_SUCCESS) return ret;

  // Wait for the operation.
  ret = libtocksync_sha_yield_wait_for_verify(correct);
