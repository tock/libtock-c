...
int main(void) {
  ...
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_check_status());

  printf("Loading in the key...\r\n");
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_set_key_buffer(key_buf, KEY_LEN));
  printf("   done\r\n");

  printf("Loading in the input buf...\r\n");
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_set_input_buffer(data_buf, DATA_LEN));
  printf("   done\r\n");

  printf("Setting up the callback...\r\n");
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_set_callback(kv_cb, &con));
  printf("   done\r\n");

  printf("Adding the key...\r\n");
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_set());
  printf("   done\r\n");
  ...
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_set_output_buffer(out_buf, DATA_LEN));
  printf("   done\r\n");

  printf("Getting the key...\r\n");
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_get());
  printf("   done\r\n");
  ...
  TOCK_EXPECT(RETURNCODE_SUCCESS, kv_delete());
  yield_for(&con);
  con = false;
  printf("   done\r\n");
