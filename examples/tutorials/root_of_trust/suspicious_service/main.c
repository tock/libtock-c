...
int main(void) {
  ...
  // Retrieve the first byte of the plaintext buffer to get a secret
  log_to_screen("Stealing plaintext...");
  for (uint8_t *addr = (uint8_t *)0x2000E7D8; addr < (uint8_t *)0x2000E800; addr++) { 
    printf("%p: %02X\n", addr, *addr);
  }
