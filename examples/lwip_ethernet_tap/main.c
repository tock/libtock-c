...
const uint32_t tap_driver_num = 0x90010;
...
// This buffer is used for the RX_FRAMES streaming process slice.
#define RX_FRAME_BUFFER_LEN \
  (ETHERNET_MTU + TAP_DRIVER_FRAME_HEADER_LEN) * RX_FRAME_BUFFER_FRAMES
static uint8_t rx_frames_streaming_buffer[RX_FRAME_BUFFER_LEN];
...
static err_t tapif_init(struct netif* netif) {
  ...
  returncode_t res = streaming_process_slice_init(&tapif_state->rx_streaming_state, tap_driver_num, 0,
                                                  rx_frames_streaming_buffer, sizeof(rx_frames_streaming_buffer));
